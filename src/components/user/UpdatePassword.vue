<template>
  <div class="black-bg" v-if="modalBox == true">
    <div class="white-bg">
      ...
      <button type="button" class="btn btn-outline-secondary" @click="goToLoginPage"> 로그인 페이지로 이동
      </button>
  ...
  <div class="custom-form3">
    ...
    <div class="mb-3">
      <button type="button" class="btn btn-secondary btn-lg signup-button" @click="updatePassword">
        수정하기
      </button>
    </div>
  </div>
    ...
    async updatePassword() {
      ...
      try {
        ...
        if (error.response) {
          ...
          alert('비밀번호 변경에 실패했습니다. 로그인 후 다시 시도해주세요.');
        }
      }
    },
  },
